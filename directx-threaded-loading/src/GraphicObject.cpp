...
GraphicObject::GraphicObject(ID3D11Device* p_d3dd, ID3D11DeviceContext* p_d3ddcontext, wchar_t* p_TextureName,  wchar_t* p_ModelName){
	HRESULT hr = S_OK;
	m_d3dd = p_d3dd;
	m_d3ddcontext = p_d3ddcontext;
	//m_Model = nullptr;
	...
	m_FXFactory.reset(new EffectFactory(p_d3dd));
	m_Model = Model::CreateFromSDKMESH(p_d3dd, p_ModelName, *m_FXFactory, true);
	m_States.reset(new CommonStates(p_d3dd));
